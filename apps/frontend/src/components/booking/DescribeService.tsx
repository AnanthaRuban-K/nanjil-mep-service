...
export default function DescribePage() {
  ...
  useEffect(() => {
    // Only check for service selection - not other data that comes later
    const checkServiceData = () => {
      try {
        ...
        console.log('Describe page - checking service data:', { service, emergency })
        
        // Only redirect if NO service is selected (this is the only required data for this step)
        if (!service) {
          console.log('No service found, redirecting to services')
        ...
        console.log('Service data loaded successfully:', { service, emergency })
      } catch (error) {
        console.error('Error accessing sessionStorage:', error)
        // If sessionStorage fails, redirect to services
        router.push('/services')
      }
    }

    // Add a small delay to ensure sessionStorage is ready
    const timer = setTimeout(checkServiceData, 100)
  ...
  const handleNext = () => {
    ...
    try {
      ...
      console.log('Description stored:', description.trim())
  ...
  const getQuickIssues = () => {
    if (selectedService === 'electrical') {
      return [
        ...
        'ஸ்விட்ச் வேலை செய்யவில்லை - Switch not responding',
      ...
      return [
        ...
        'சிங்க் அடைத்து போய்விட்டது - Sink is blocked',
        'பாத்ரூமில் தண்ணீர் தேங்குகிறது - Water pooling in bathroom'
      ]
    }
    return []
  }
          ...
          <div className="mb-6">
            ...
            <div className="flex justify-between items-center mt-2">
              <span className="text-sm text-gray-500">
                குறைந்தது 10 எழுத்துக்கள் • Minimum 10 characters
              </span>
              <span className="text-sm text-gray-500">
